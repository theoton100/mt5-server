...
$wine_executable python -m pip install --no-cache-dir "MetaTrader5==$metatrader_version" mt5linux python-dateutil

# Install bridge on host side
pip3 install --break-system-packages --no-cache-dir mt5linux rpyc plumbum numpy
...
if ss -tuln | grep ":$mt5server_port" >/dev/null; then
    ...
    log "[7/7] WARNING: bridge not bound yet on $mt5server_port."
fi
